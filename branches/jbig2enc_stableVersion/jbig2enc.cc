...
int removeTemplates(jbig2ctx * ctx, std::list<int> templatesToRemove) {
  ...
  for (int i = (pixat->n - 1); ((i > 0) && (it != templatesToRemove.end())); i--) {
    int index = i;

    // when testing check if it is enough to use iterator actual position or not
    if (binary_search(it, templatesToRemove.end(),i)) {
      PIX * endPix;
      PIX * coppiedPix;
      BOXA * boxa;
      int newIndex = (*it);
...
void autoThreshold(struct jbig2ctx *ctx) {
  ...
  for (int i = 0; i < pixaGetCount(jbPixa); i++) {
    PIX *jbPix = jbPixa->pix[i];
    getBoxOfFirstInstanceOfTemplate(ctx, i);
