...
/*
  l_uint16 wpl = pixGetWpl(pix);

  fprintf(stderr, "print as l_uint32 per row\n");
  l_uint32 *data = pixGetData(pix);
  for (l_uint16 i = 0; i < h; i++) {
    for(l_uint8 j = 0; j < wpl; j++) {
      printbitssimple(data[i*wpl+j]);
    }
    fprintf(stderr,"\n");
...
int areEquivalent(PIX *const firstTemplate, PIX *const secondTemplate) {
  ...
  l_int32 vlineThresh = (verticalPart * 3.5);
  l_int32 hlineThresh = (horizontalPart * 3.5);
  ...
  // checking for horizontal lines
    ...
    for (int j = 0; j < (divider-1); j++) {
      ...
      if (horizontalSum > hlineThresh) {
        freeMatrix(parsedPixCounts, divider);
        freeMatrix(horizontalParsedPixCounts, divider*2);
        freeMatrix(verticalParsedPixCounts, divider);
        return 0;
      }
    }
  }
